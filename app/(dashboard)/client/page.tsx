import { ShoppingCart, ClipboardList, CheckCircle, Clock, Sparkles } from 'lucide-react'
...
export default async function ClientOverviewPage() {
  ...
  const placedOrders = orders.filter((o) => o.status !== 'cart')
  const active = placedOrders.filter((o) =>
    ['queued', 'claimed', 'processing'].includes(o.status)
  )
  const submitted = placedOrders.filter((o) => o.status === 'submitted')
  const delivered = placedOrders.filter((o) => o.status === 'delivered')
  const totalSpent = delivered.reduce((sum, o) => sum + o.total_amount, 0)

  const firstName = profile?.full_name?.split(' ')[0] ?? 'there'
  const hasActivity = placedOrders.length > 0

  return (
    <div>
      <Header
        title={`Welcome, ${firstName}`}
        subtitle={hasActivity ? "Here's what's happening with your orders" : 'Your portal is ready'}
      />

      <div className="p-6 space-y-6">
        {hasActivity ? (
          <>
            {/* Stats — only shown once the client has placed at least one order */}
          ...
          /* First-time / no-activity state — clean blank welcome */
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-gray-500 max-w-sm mb-8">
              Browse our services and place your first order to get started with a consultant.
            </p>
